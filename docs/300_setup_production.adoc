...
2. Create a ssh key pair with
    ...
    ```
    ssh-keygen -t ed25519 -f /to/project/ssh/dir/of/project/ansible/path -C "admin@amova.eu"```

    # For eaxample
    ssh-keygen -t ed25519 -f /mnt/c/IDE/Projects_Git/ADI-Bourbon/adi-bourbon/wms-adibourbon/src/ansible/environment/prod/inventory/ssh/id_rsa -C "admin@amova.eu"
    ```
...
# Allow remote user access with password if nedded
